...
- 自動ゲート結果と人の判断ログと最終サマリを統合して保管する。
- 人の判断ログではAbortの理由を必須とし、Doneは理由不要とする。
...
- 証跡ハブ（Allure）は自動ゲート結果と人の判断ログと最終サマリを統合して保管し後追い説明可能性を担保する。
...
- ワークフローツール最終工程ではCI成果物のzip等アップロードを必須にする。
- 同工程でCoach相当のチェックリスト入力を確定し合計点を算出する。
- 判定結果はpassとwarnとfailの三値で出力しAllureへ統合する。
- 各ゲートと各評価軸のスコアは機械可読な成果物として継続比較できる形式で保存する。
- 自動ゲート結果と人の判断ログと最終サマリを単一の証跡として保管する。
- ワークフローツール最終工程ではチェックリスト（Coach相当）の入力と承認を行いCI成果物をアップロードする。
- 集約処理では合計点（自動＋チェックリスト）を算出しpass|warn|failで出力する。
- 証跡ハブ（Allure）には自動ゲート結果と人の判断ログと最終サマリを統合して保管する。
- ワークフローツール（最終工程）ではCI成果物（zip等）をアップロードして最終サマリを1枚に集約する。
- 同工程でチェックリスト（Coach相当）の入力を確定し合計点（自動＋チェックリスト）を算出する。
- 判定結果はpass|warn|failで出力し0.70未満は必ずWarningを出す。
- 証跡ハブ（Allure）では自動ゲート結果と人の判断ログと理由と証跡リンクとサマリを統合して保管する。
- ブロック（Fail）条件は重大ゲートに限定して段階的に導入する。
